...
import { BoolField, TagField, type TagFilterItem } from '$lib/types/filter-enums';
...
const emptyTagFilters: [TagField, SvelteSet<string>][] = Object.values(TagField).map(tagField => [
    tagField,
    new SvelteSet(),
]);
...
export class FicCardsPageState {
    ...
    public hasIncluded = $derived(
        ![...this.appliedFilters.includedTagFilters].every(([_, values]) => ![...values].length)
    );
    public hasExcluded = $derived(
        ![...this.appliedFilters.excludedTagFilters].every(([_, values]) => ![...values].length)
    );
