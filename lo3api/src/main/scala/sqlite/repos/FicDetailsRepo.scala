...
class FicDetailsRepo(db: Lo3Db):
  ...
  private def getDetailsOption(key: UserFicKey): IO[DbError, Option[FicDetails]] = for {
    docs <- db.run(filterDetails(key).result)
  } yield docs.headOption.map(_.toModel)

  private def addUserFicRecord(key: UserFicKey): IO[DbError, FicDetails] = {
    val newDetails = FicDetailsDoc(
      id = None,
      userId = key.userId,
      ficId = key.ficId,
    ...
    for {
      newId <- db.run(
        (ficsDetails returning ficsDetails.map(_.id)) += newDetails
      )
    } yield newDetails.copy(id = Some(newId)).toModel
  }
