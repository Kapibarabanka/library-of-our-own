...
import kapibarabanka.lo3.common.models.domain.{DbError, FicComment, UserFicKey}
...
class CommentsRepo(db: Lo3Db):
  ...
  def addComment(key: UserFicKey, comment: FicComment): IO[DbError, Unit] = db
    .run(
      comments += CommentDoc(None, key.userId, key.ficId, key.ficIsSeries, comment.commentDate, comment.comment)
    )
    .unit

  def getAllComments(key: UserFicKey): IO[DbError, List[FicComment]] = for {
