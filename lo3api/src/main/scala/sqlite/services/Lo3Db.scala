...
class Lo3Db(dbWithPath: String):
  ...
  def run[T](action: DBIOAction[T, NoStream, Nothing]): IO[DbError, T] = {
    ...
    } yield db).mapError(e => DbError(e.getMessage))
    ...
    def use(db: JdbcDatabaseDef) = ZIO.fromFuture { implicit ec => db.run(action) } mapError (e => DbError(e.getMessage))
