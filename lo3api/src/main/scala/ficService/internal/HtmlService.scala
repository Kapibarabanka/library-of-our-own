...
class HtmlService(ao3: Ao3HttpClient):
  ...
  def work(id: String): IO[Lo3Error, WorkHtml] =
    getDoc(Ao3Url.work(id), true, s"work with id $id")(html => WorkHtml(html))
    ...
    allPages <- firstPage.pageCount match
      ...
      case Some(pageCount) =>
        ZIO.collectAll(
          ...
          yield getDoc(Ao3Url.seriesPage(id, i), true, entityName)(html => SeriesPageHtml(html, id, i))
        )
  } yield allPages.toList

  def seriesFirstPage(id: String): IO[Lo3Error, SeriesPageHtml] =
    getDoc(Ao3Url.series(id), true, s"series with id $id")(html => SeriesPageHtml(html, id, 1))

  def tag(id: String): IO[Lo3Error, TagHtml] = getDoc(Ao3Url.tag(id), false, s"tag '$id'")(html => TagHtml(html))
